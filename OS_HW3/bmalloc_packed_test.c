...
void *bmalloc(size_t s)
{
	...
	// resize the requested array to accomodate the new request
	requested = realloc(requested, (request_count + 1) * sizeof(_request));
	...
	if (bm_mode == FirstFit)
	{
		...
		{ // Find the proper one, so allocate
			...
			requested[request_count].size = s;
			requested[request_count].addr = addr + sizeof(bm_header);
			request_count++;
			cleaning(addr);
...
void bfree(void *p)
{
	...
	if (itr == NULL)
	{
		printf("\nfree error, you may requested to free not allocated memory.\n");
		exit(1);
	}
	...
	_request *temp;
	temp = (_request *)malloc(sizeof(_request) * request_count);
	for (int i = 0, j = 0; i < request_count; i++)
	{
		if (requested[i].addr != p)
		{
			memcpy(&temp[j], &requested[i], sizeof(_request));
			j++;
		}
	}
	request_count--;
...
{
	bm_header_ptr itr;
	size_t given = 0, available, inFrag = 0, sumRequested = 0;
	int i, j;
	...
	for (itr = bm_list_head.next, i = 0; itr != 0x0; itr = itr->next, i++)
	{
		...
		if (itr->used == 1)
		{
			...
			{
				if (requested[j].addr == ((void *)itr + sizeof(bm_header)))
				{
					sumRequested += requested[j].size;
				}
			}
			given += (1 << itr->size);
		}
	...
	printf("The total amount of all given memory : %d bytes\n", 4096 * page_count);
	...
	available = 4096 * page_count - given;
	printf("The total amount of available memory : %zu bytes\n", available);
	// total amount of the internal fragmentation = payload - request
	inFrag = given - sizeof(bm_header) * request_count - sumRequested;
	printf("The total amount of the internal fragmentation : %zu bytes\n", inFrag);
